...
def Calculate_Q_exact(m_AB, T):
    Boltzmann_factors = [exp(-E/T) for E in m_AB] #Should return a list of all Boltzmann factors (e^(-E/T))
...
def Calculate_Q_approx(Number_of_particles, T):
    W = factorial(Number_of_particles)/(factorial(Number_of_particles/2)*factorial(Number_of_particles/2))  #Should return the multiplicity of a system with N particle and N_A = N_B = N/2
